...
CSV_COLUMNS = [
    ...
    "inactive_90d",          # TRUE if never logged in, or last login > 90 days ago
    "tickets_assigned",      # only populated when --check-ticket-activity runs
    "tickets_submitted",
    "reclaim_candidate",     # TRUE only if dormant AND no ticket activity found
    "organization_id",
]

# A user who has never logged in counts as inactive: there is no evidence the
# seat is being used. Kept as a named constant so the threshold is easy to change.
INACTIVITY_DAYS = 90
...
def count_search(session: requests.Session, base: str, query: str) -> int:
    """
    GET /api/v2/search/count returns just a total, no result bodies. Much
    cheaper than paging /search when all we need is "did anything happen".
    """
    data = get_json(session, f"{base}/api/v2/search/count", {"query": query})
...
def ticket_activity(session: requests.Session, base: str, user_id: int,
    ...
    """
    Return (tickets_assigned, tickets_submitted) touched since `since`.
    ...
    assigned = count_search(session, base, f"type:ticket assignee:{user_id} updated>{day}")
    time.sleep(SEARCH_PAUSE)
    submitted = count_search(session, base, f"type:ticket submitter:{user_id} updated>{day}")
    time.sleep(SEARCH_PAUSE)
...
def login_recency(last_login_at: str | None, now: datetime, threshold: int) -> tuple[str, str]:
    ...
    A null last_login_at means the user has never signed in. That is reported as
    a blank day count but still flags as inactive - an unused seat is an unused
    seat whether it was abandoned or never taken up.
    """
...
def main() -> None:
    ...
    p.add_argument("--check-ticket-activity", action="store_true",
                   help="For dormant seats only, query the Search API for ticket activity in the "
                        "same window. Adds 2 API calls per dormant user. Sets reclaim_candidate.")
    args = p.parse_args()
    ...
    if args.check_ticket_activity:
        ...
        dormant_rows = [r for r in rows if r["inactive_90d"] == "TRUE"]
        print(f"\nChecking ticket activity for {len(dormant_rows)} dormant seat(s) "
              f"since {cutoff:%Y-%m-%d} ({len(dormant_rows) * 2} API calls)...", file=sys.stderr)
        for i, r in enumerate(dormant_rows, 1):
            assigned, submitted = ticket_activity(session, base, r["id"], cutoff)
            r["tickets_assigned"] = assigned
            r["tickets_submitted"] = submitted
            r["reclaim_candidate"] = "TRUE" if (assigned + submitted) == 0 else "FALSE"
            flag = "no activity" if r["reclaim_candidate"] == "TRUE" else f"{assigned + submitted} ticket(s)"
            print(f"  [{i}/{len(dormant_rows)}] {r['name'][:34]:<34} {flag}", file=sys.stderr)
    ...
    print(f"\nDormant (no login in {args.inactivity_days}+ days): {dormant} of {len(rows)}"
          f"  [{never} have never logged in]", file=sys.stderr)

    if args.check_ticket_activity:
        confirmed = sum(1 for r in rows if r["reclaim_candidate"] == "TRUE")
        contradicted = sum(1 for r in rows if r["reclaim_candidate"] == "FALSE")
        print(f"  no ticket activity either: {confirmed}", file=sys.stderr)
        ...
        print("\nNote: search covers assignee and submitter only. Light Agents who just "
              "comment will show zero. Confirm with role owners before removing anyone.",
              file=sys.stderr)
