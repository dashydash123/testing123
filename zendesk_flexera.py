...
--check-ticket-activity, --inactive-only, --inactivity-days, --out,
--active-only) are accepted and ignored, so one ZENDESK_DEFAULT_ARGS line can
serve both scripts.
...
.env (next to this script):
    ...
    ZENDESK_DEFAULT_ARGS=--all-time --exclude-seat-class "Agente Light"
...
EXTRACT_ONLY_FLAGS = {"--exclude-end-users": 0, "--check-ticket-activity": 0,
                      "--inactive-only": 0, "--active-only": 0,
                      "--inactivity-days": 1, "--out": 1}
