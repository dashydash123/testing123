...
def run_anaplan_export():
    ...
    required = {"WORKSPACE_ID": WORKSPACE_ID, "MODEL_ID": MODEL_ID,
    ...
    if missing:
        sys.exit(f"Set these Anaplan values before running: {', '.join(missing)}")
...
def build_flexera_frames(raw: pd.DataFrame):
    ...
    assignments = pd.DataFrame({
        "Email ID":              raw["email"],
        "User ID":               raw["user_id"],
        "License Type":          raw["license_type"].str.strip(),
        "Account Creation Date": created,
        "Last Active Date":      last_active,
