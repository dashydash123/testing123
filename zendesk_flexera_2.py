...
Users who have never logged in are sent in both files. Flexera warns on
their activity row and skips only that row. Suspended users are sent as-is.
...
def main() -> None:
    ...
    no_key = [r for r in rows if not r["id"] or not r["email"]]
    rows = [r for r in rows if r["id"] and r["email"]]
    ...
    if no_key:
        print(f"  {len(no_key)} user(s) skipped: no id/email")
    if filled:
